namespace zj {
	 ...
	 * 2019.11.7
	 */
	export class AdventureDialogItemElite extends UI {
		private static colorPass: number = 0x16fa5b;
		private static colorCurr: number = 0xe84f33;
		private static colorNo: number = 0xaaaaaa;
		...
		private currChapter: TableChapterElite;
		...
		public constructor() {
			super();
			this.skinName = "resource/skins/adventure/AdventureDialogItemEliteSkin.exml";
		...
		public init(father, area: TableInstanceArea, chapter: TableChapterElite, data: TableInstance, idx: number) {
			this.father = father;
			this.currArea = area;
		...
		private getCurrInstanceInfo(): CustomInstanceInfo{
			return Game.PlayerInstanceSystem.curInstances[message.EFormationType.FORMATION_TYPE_INSTANCE_ELITE];
		}
		public updateUI() {
		...
		private onBtnDekaron() {
			...
			if (power < num) {
				...
				Game.PlayerInstanceSystem.MobsInfo_Req(id)
					.then((value: {}) => {
						Game.PlayerInstanceSystem.curInstanceType = message.EFormationType.FORMATION_TYPE_INSTANCE_ELITE;
