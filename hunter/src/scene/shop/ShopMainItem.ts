...
//hexiaowei
//2018/12/21
export class ShopMainItem extends eui.ItemRenderer {
    ...
    public constructor() {
        super();
        this.skinName = "resource/skins/shop/ShopMainItemSkin.exml";
        cachekeys(<string[]>UIResource["ShopMainItem"], null);
        // 碎片遮罩
    ...
    protected dataChanged() {
        closeCaches([this.groupAnimate, this.groupCache]);
        this.rectMaskCommon.visible = false;
        this.rectMask.visible = false;
        this.imgMask.visible = false;
        ...
        setCaches([this.groupAnimate, this.groupCache]);
    }
    ...
    private setInfoRemain(data: ShopMainItemData) {
        ...
        if (data.mallitem.buy_limit > Game.PlayerInfoSystem.Level) {
            ...
            let remain = data.shopmallscene.getMallRemain(data.mallitem.mall_id)
...
//子项数据源
export class ShopMainItemData {
    //数据源
    mallitem: message.MallItem;

    //父类 //
    shopmallscene: ShopMallDialog;
